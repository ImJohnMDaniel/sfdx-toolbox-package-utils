...
export default class Install extends SfdxCommand {
  ...
  private processDebugMessage( debugMessage: any): void {
    if ( debugMessage !== undefined ) {
      this.debug( 'DEBUG ' + debugMessage);
    }    
  }
  private processDebugMessages( debugMessages: any[]): void {
    if ( debugMessages !== undefined ) {
      for( let aMessage of debugMessages ){
        this.processDebugMessage(aMessage);
      }
    }
  }
  ...
  public async run(): Promise<any> { // tslint:disable-line:no-any
    ...
    this.processDebugMessage('before call to this.project.retrieveSfdxProjectJson()');
    const projectJson = await this.project.retrieveSfdxProjectJson();
    this.processDebugMessage('after call to this.project.retrieveSfdxProjectJson()');
