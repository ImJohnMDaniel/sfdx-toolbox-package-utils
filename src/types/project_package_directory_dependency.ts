...
export class ProjectPackageDirectoryDependency {
    ...
    public setPackageAndVersionNumber(package2Id: string, versionNumber: string) {
    ...
    public setSubscriberPackageVersionId(subscriberPackageVersionId: string) {
        this.subscriberPackageVersionId = subscriberPackageVersionId;
    }

    private processProjectPackageDirectoryDependencyJson(projectPackageDependency: AnyJson) {
        ...
        const { package: dependentPackageAnyJson, versionNumber: versionNumberAnyJson } = projectPackageDependency as JsonMap;
        const dependentPackageStr: string = JSON.stringify(dependentPackageAnyJson) ? JSON.stringify(dependentPackageAnyJson).replace('"', '').replace('"', '') : undefined;
        const versionNumberStr: string = JSON.stringify(versionNumberAnyJson) ? JSON.stringify(versionNumberAnyJson).replace('"', '').replace('"', '') : undefined;
        ...
        } else if (dependentPackageStr.startsWith(Constants.PACKAGE_ID_PREFIX)) {
